            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.A(sitterDetailButton, href='/ask-sitter')
                        ])
            ]
        )
    ]
...
# ask-sitter page content
page_ask_sitter = html.Div(
    children=[
        html.H1('This is ask_sitter page.'),
        html.Div(
            className="ask-sitter--left-panel",
            children=[]
        ),
        html.Div(
            className="ask-sitter--right-panel",
            children=[
                html.Img(id='ask-sitter-image'),
                html.H3(id='ask-sitter-name'), 
                html.H4(id='ask-sitter-stars'), 
                html.H5(id='ask-sitter-age'),
                html.P(id='ask-sitter-comment'),
            ])])
...
def display_page(pathname, n_clicks, children):
    print("Button clicked")
    print("Carried over value: ", children)
    """
    if pathname == '/':
        return init_page
    elif pathname == '/ask-sitter':
        return page_ask_sitter
    else:
        return '404'
    """
...
def show_desc(data):
    if data is None:
        ...
        request_link = dcc.Link('Request a pet-sitting', href='/ask-sitter')
        intermediate_value = {"id":id, "name":name, "age":age, "comment":comment, "stars":stars}
