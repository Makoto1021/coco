...
datePicker = dcc.DatePickerSingle(
    id="my-date-picker-single", date=today, className="app-body--datepicker"
)
...
sitterDetailButton = html.Button(
    "Request a pet-sitting", id="sitter-detail-button", n_clicks=0
)
...
layout = html.Div(
    ...
    children=[
        html.Div(
            html.H1(children="Find a pet-sitter nearby!"), className="app-header--title"
        ),
        html.Div(
            className="app-body",
            id="app-body",
            children=[
                html.Div(
                    ...
                    children=[
                        html.Div(datePicker, className="app-body--datepickerContainer"),
                        html.Div(
                            className="app-body--startTimeContainer",
                            children=[html.Label("Start:"), startTime],
                        ),
                        html.Div(
                            className="app-body--endTimeContainer",
                            children=[html.Label("End:"), endTime],
                        ),
                        html.Div(searchButton, className="app-body--searchbutton"),
                        html.Div(dcc.Graph(id="map"), className="app-body--map"),
                    ],
                ),
                html.Div(
                    ...
                    children=[
                        html.Div(
                            html.Img(id="image", className="app-body--desc--image"),
...
# callbacks
@app.callback(Output("map", "figure"), Input("search-button", "n_clicks"))
    ...
    if n_clicks == 0:
        ...
        fig.update_layout(
            ...
            mapbox=dict(
                accesstoken=TOKEN,
                bearing=0,
                center=dict(lat=48.880460, lon=2.309019),
                pitch=0,
                zoom=12,
            ),
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                lon=[2.313587],
                mode="markers",
                marker=go.scattermapbox.Marker(size=12, color="red", symbol="dog-park"),
                text="You are here!",
                hoverinfo="text",
            )
        )
...
def show_desc(data):
    if data is None:
        ...
        request_link = dcc.Link("See details", href="/apps/pet-sitters")
        intermediate_value = {
            "id": id,
            "name": name,
            "age": age,
